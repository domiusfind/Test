...
-- Fluent UI compatibility: global flags registry
Library.Flags    = {}  -- all controls indexed by their ID
Library.Toggles  = Library.Flags  -- alias
Library.Options  = Library.Flags  -- alias
		...
		function pageFunction:AddSection(Section_Name, Toggleable, SectionGap, SectionColor)
			...
			function sectionFunction:AddToggle(idk,Setting)
				...
				local function ButtonClick()
				    ...
				    for _, fn in ipairs(_toggleCallbacks) do pcall(fn, Default) end
				end
				...
				-- Register in global Flags by ID
				if idk and idk ~= "" then
					Library.Flags[idk] = toggleFunction
				end

				return toggleFunction
			end
        function sectionFunction:AddButton(idOrSetting, Setting)
        	-- Fluent UI: AddButton("Id", {Setting}) OR classic: AddButton({Setting})
        	local id = nil
        	if type(idOrSetting) == "string" then
        		id = idOrSetting
        	else
        		Setting = idOrSetting
        	end
            ...
            TextColor_1.Parent = RowBG_1
             ...
             Button_1.MouseLeave:Connect(function()
                ...
                function f:OnChanged(fn)
                    table.insert(_btnCallbacks, fn)
                end
                -- Register in global Flags by ID
                if id and id ~= "" then
                    Library.Flags[id] = f
			...
			function sectionFunction:AddDropdown(idk, Setting)
				...
				-- Fluent UI: .Value property + :OnChanged()
				local _dropCallbacks = {}
				dropdownFunction.Value = Default or ""

				local _origDropSetValue = dropdownFunction.SetValue
				function dropdownFunction:SetValue(value)
					_origDropSetValue(self, value)
					self.Value = self:GetValue()
					for _, fn in ipairs(_dropCallbacks) do pcall(fn, self.Value) end
				end
				function dropdownFunction:OnChanged(fn)
					table.insert(_dropCallbacks, fn)
				end

				-- Register in global Flags by ID
				if idk and idk ~= "" then
					Library.Flags[idk] = dropdownFunction
				end
				local controlData = {
                    Name = Title,
                    Section = Section,
    ...
    return keybindFunction
			...
			function sectionFunction:AddInput(idk, Setting)
				...
				Boxxx.FocusLost:Connect(function()
					...
					if Boxxx.Text ~= '' then
						textbox_function.Value = Boxxx.Text
						if Callback then Callback(Boxxx.Text) end
						for _, fn in ipairs(_inputCallbacks) do pcall(fn, Boxxx.Text) end
					end
				end)
				local textbox_function = {}
				local _inputCallbacks = {}
				textbox_function.Value = Default or ""
				...
				function textbox_function.SetValue(Value)
					Boxxx.Text = Value
					textbox_function.Value = Value
					if Callback then Callback(Value) end
					for _, fn in ipairs(_inputCallbacks) do pcall(fn, Value) end
				end
				function textbox_function:OnChanged(fn)
					table.insert(_inputCallbacks, fn)
				end

				-- Register in global Flags by ID
				if idk and idk ~= "" then
					Library.Flags[idk] = textbox_function
				end
				local controlData = {
                    Name = TitleText,
                    Section = Section,
			...
			function sectionFunction:AddSlider(idOrSetting, Setting)
				-- Fluent UI: AddSlider("Id", {Setting}) OR classic: AddSlider({Setting})
				local id = nil
				if type(idOrSetting) == "string" then
					id = idOrSetting
				else
				...
				local callBackAndSetText = function(val)
					Sliderbox_2.Text = val
					if slider_function then
						slider_function.Value = tonumber(val)
						for _, fn in ipairs(_sliderCallbacks) do pcall(fn, tonumber(val)) end
					end
				...
				local slider_function = {}
				slider_function.Value = DefaultValue

				function slider_function.SetValue(Value)
					GetSliderValue(Value)
				end
				function slider_function:OnChanged(fn)
					table.insert(_sliderCallbacks, fn)
				end
				local controlData = {
                    Name = TitleText,
                    Section = Section,
				...
				-- Register in global Flags by ID
				if id and id ~= "" then
					Library.Flags[id] = slider_function
				end
